...
任务分析器是路由的输入端。Agent 提交任务后，分析器用一个轻量 LLM（默认 Gemini，备选 DeepSeek）做结构化分析：判断任务属于什么领域，在 coding、reasoning 等维度上难度各多少分，并尝试归类到 scrape、summarization、classification 这些预定义类别。分析器有 fallback 链，LLM 全挂了就退到基于规则的启发式分析，保证不阻塞。

路由引擎是核心。它拿到分析结果和候选模型后做多层决策：先按能力维度匹配度和成本做综合打分排序；再查 class pool——每类 Agent 都有历史表现记录，用 Wilson 置信下界算成功率给表现好的模型加分，用得越多越准；同时检查模型健康状态和速率限制，不健康或快被限流的模型会被降权。引擎还有两个进阶机制：降级优化会在表现稳定时自动尝试用更便宜的模型替代，成功就永久切换，失败就回滚；升级逃逸则是在执行失败时沿候选列表向上尝试更强的模型。

监控模块旁路记录每次路由决策和执行结果，支持统计查询和实时观察，既用于排查问题，也为路由策略优化提供数据。

总的来说，用便宜模型先理解任务需要什么能力，再从候选池中结合能力画像、历史战绩、实时状态和成本约束选出最优模型，通过执行反馈持续学习，在不牺牲质量的前提下压低 Agent 的调用成本。
...
- **B6**：高并发限流（工程实现细节）：Redis 模式用滑动窗口统计 RPM/RPD（有序集合裁剪+计数）与并发计数（自增/自减+TTL），并区分正常流量与升阶流量设置升阶并发封顶；提供利用率缓存与本地内存降级模式，Redis 不可用时按策略自动切换，保证路由服务持续可用；并发压测流程与成绩（可复现 A/B）：基于 `route_agent/router_engine/tests/perf/test_batch_concurrency_allocation_perf.py` 与 `scripts/perf_ab_compare.py`，使用 12 个合成模型（4 providers，带精确 RPM/并发上限）+ 5 批×4=20 agents（固定种子 `SEED=20260224`）重叠调度（`asyncio`），对照组固定单模型（`control_strategy=pin_model` → `google:gemini-2.5-pro`）并在超限时进行有界指数退避重试（最大 200 次、单请求最大排队 60s、总 wall time 120s、并发重试节流 8），两组重复运行 3 次取中位数：实验组（重叠批次）`elapsed_seconds=20.625`、`throughput_rps=0.97`、`success_throughput_rps=0.97`、`success_rate=1.0`、`rejection_rate=0.0`、`completion_all_p95=14684.15ms`、`allocatable/eligible/assigned=12/12/12`；对照组 `elapsed_seconds=66.11`、`throughput_rps=0.303`、`success_throughput_rps=0.303`、`success_rate=1.0`、`rejection_rate=0.929`、`completion_all_p95=61437.85ms`、`allocatable/eligible/assigned=12/1/1`，并且窗口锚点固定为 `t0=min(submit_ts)`（`window_start_ts=0.0`、`window_end_ts=60.0`）可直接复算窗口指标；超时边界与指标定义口径详见 Q20。  
