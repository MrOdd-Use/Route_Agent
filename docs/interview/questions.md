...
任务分析器是路由的输入端。Agent 提交任务后，分析器用一个轻量 LLM（默认 Gemini，备选 DeepSeek）做结构化分析：判断任务属于什么领域，需要模型的什么能力，能力复杂度为多少，并尝试归类到预定义的任务类别。分析器有 fallback 链，LLM 全挂了就退到基于规则的启发式分析，保证不阻塞。
